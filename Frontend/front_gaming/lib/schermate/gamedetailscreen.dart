...
class _GameDetailScreenState extends State<GameDetailScreen> {
  ...
  Future<void> toggleAchievement(int index) async {
    ...
    final url = Uri.parse(
      "http://localhost:8000/user/$userId/game/${widget.game.gameId}/achievement/$index/toggle_achieved",
    );
  ...
  Future<String?> fetchSviluppatoreLogo(String? name) async {
    ...
    final uri = Uri.parse(
        'http://localhost:8000/company_logo?name=${Uri.encodeComponent(name)}');
    try {
